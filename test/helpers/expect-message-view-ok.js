...
module.exports = ($, data) => {
  ...
  expect(formGroups).toHaveLength(8)
  ...
  expect($('legend', formGroups.eq(2)).text()).toMatch('Do you want to send the message to all organisations?')
  expect($('legend', formGroups.eq(3)).text()).toMatch('Select the individual organisations to send the message')
  expect($('#officeCodes', formGroups.eq(4)).text()).toMatch('Which office locations should the message be sent to?')
  expect($('#allOffices').val()).toEqual('true')
  expect($('legend', formGroups.eq(5)).text()).toMatch('Do you want to send the message to all office locations')
  expect($('.govuk-checkboxes__item', formGroups.eq(3))).toHaveLength(data.orgList.length)
